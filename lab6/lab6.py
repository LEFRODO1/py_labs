...
def pingpong(n):
    ...
    if not n:
        return 0
    elif '8' in str(n) or not (n % 8):
        return pingpong(-n - 1 if n < 0 else -n + 1) - (1 if n < 0 else -1)
    else:
        return pingpong(n - 1 if n > 0 else n + 1) - (-1 if n < 0 else 1)
...
def missing_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    if n and n // 10 and n % 10 != (n % 100 // 10):
        return n % 10 - (n % 100 // 10) + missing_digits(n // 10) - 1
    elif n and n // 10 and n % 10 == (n % 100 // 10):
        return n % 10 - (n % 100 // 10) + missing_digits(n // 10)
    elif n:
        return 0
